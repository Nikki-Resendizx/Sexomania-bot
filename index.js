...
>═══◄••𝙉𝙊𝙈𝘽𝙍𝙀 𝘿𝙀𝙇 𝘾𝙃𝘼𝙏••►═══
》 ${c.nombre.replace(/#g|#r|#p|#y/g,'').trim()} 《

>═══◄•• 𝘿𝙀𝙎𝘾𝙍𝙄𝙋𝘾𝙄𝙊𝙉 ••►═══

${c.desc}

>📊 𝘼𝙇𝘾𝘼𝙉𝘾𝙀 𝙏𝙊𝙏𝘼𝙇⠅ 👁️ ${c.clicks||0} 𝚅𝙸𝚂𝚃𝙰𝚂
...
async function mandarSeccion(sec, ctx){
  ...
  let btns=[];
  snap.forEach(d=>{
    cacheChats[d.id]={id:d.id,...d.data()};
    btns.push([Markup.button.callback(`${textoSeguro(d.data().nombre)} | ${d.data().clicks||0}`,`ver_${d.id}`)])
  });
...
// PANEL ADMIN + COMANDO DE COLORES
bot.command('admin', async (ctx)=>{
  if(ctx.from.id!=8695673050) return ctx.reply('⛔ No eres admin');
  await ctx.reply(`⚙️ *PANEL ADMIN SEXOMANIA*\n\n🎨 Para colores pon al final del nombre:\n#g = 🟢 #r = 🔴 #p = 🔵 #y = 🟡\nEj: Sexomania VIP #r`,
  {parse_mode:"Markdown",...Markup.inlineKeyboard([
    ...
    [Markup.button.callback('🎨 VER COLORES','cmd_colores')],
    [Markup.button.callback('👁️ Ver menu','volver_menu')]
  ])});
});

bot.command('colores', async (ctx)=>{
  if(ctx.from.id!=8695673050) return ctx.reply('⛔ Solo admin');
  return ctx.reply(
`🎨 *COMANDO DE COLORES - SEXOMANIA*

Para cambiar el color del boton, edita el NOMBRE del chat en Firebase y pon al final:
...
*EJEMPLOS:*
\`Sexomania VIP #r\` -> 🔴 Sexomania VIP
\`Grupo XXX #g\` -> 🟢 Grupo XXX
\`Ventas VIP #y\` -> 🟡 Ventas VIP
\`Arte #p\` -> 🔵 Arte

Si no pones nada, sale normal sin color.

El color se vera en el boton del menu y en la plantilla se limpia automatico.`,
{parse_mode:"Markdown"}
  );
});

bot.action('cmd_colores', async (ctx)=>{
  await ctx.answerCbQuery().catch(()=>{});
  return ctx.reply(
`🎨 *COLORES DISPONIBLES*
...
bot.action(/^cfg_/, async (ctx)=>{
  if(ctx.from.id!=8695673050) return;
  ...
  await ctx.reply(`✅ Guardado: ${d}`,getMenuInline());
});
...
(async()=>{ await bot.telegram.deleteWebhook().catch(()=>{}); await bot.launch(); console.log('BOT CON TU PLANTILLA Y COLORES ON'); })();
